...
| Security      | `security-reviewer` (Opus) | non-test code in `src/main`, `src/preload`, `src/shared`, `src/core`, and renderer `.ts`/`.tsx`/`.html`; dependencies and `.npmrc`; build, packaging, TypeScript, Vitest, and lint config; `.gitattributes`; `.nvmrc`; CI; `resources/`; the review gate (`.githooks/`, `.claude/review-gate/`); `scripts/`; and `.claude/settings*.json`, `.claude/hooks/`, and `.claude/agents/` |
| Accessibility | `a11y-reviewer` (Sonnet)   | renderer `.tsx`, `.css`, or `.html`                                                                                                                                                                                                                                                                                                                                                |

Docs-only diffs (Markdown outside `.claude/agents/`, anything in `.claude/skills/`, `LICENSE`) skip all three. Test-only diffs get a code review only. Run `node .claude/review-gate/cli.mjs plan` for the exact answer on a staged diff. The gate's path rules (`.claude/review-gate/lib/pathClassification.mjs`) are the source of truth.
...
5. Record the receipt with `node .claude/review-gate/cli.mjs record`, then commit.

**Git hooks enforce this once you opt in.** Run `node .claude/review-gate/cli.mjs install` once per clone. It points `core.hooksPath` at `.githooks/` (unless this clone already sets it to something else, in which case it leaves that value and exits with an error). The hook there runs the gate code in `.claude/review-gate/`. From then on, git runs the gate for `git commit`, whatever command starts it, including in linked worktrees that have `.githooks/` checked out. Merge commits aren't gated, because every commit on the merged branches already passed the gate. While a merge is in progress (`MERGE_HEAD` exists), anything staged lands unreviewed, including conflict resolutions. Stage only the merge result, and review conflict resolutions yourself. The gate blocks until a receipt matching the exact index being committed exists (stored at `<git-dir>/beekeeper-review-receipt`) and the checks pass. Docs-only commits pass without a receipt, and any error inside the gate blocks the commit. The default reviewers live in `.claude/agents/`.

Git doesn't run the hook for `cherry-pick`, `revert`, `rebase`, `am`, or `commit-tree`, and `--no-verify` skips them. Don't use those to land unreviewed changes. In Claude Code, a guard hook (`.claude/hooks/block-hook-bypass.mjs`) catches the common bypasses. It's a speed bump, not a wall. CI runs the lint, format, typecheck, test, and build checks but not these reviews, so the gate is a habit, not an enforced control.
